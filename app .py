...
hook_data = pd.DataFrame({
    ...
    'Percentage': [
        depressed/total*100, sleep_issues/total*100, validation/total*100,
        len(filtered_df[filtered_df['Easy_Distraction'] >= 4])/total*100,
        high_usage/total*100
    ]
})
fig_hook = px.bar(hook_data, x='Issue', y='Percentage',
...
fig_hook.update_layout(plot_bgcolor='#0d0d0d', paper_bgcolor='#0d0d0d',
